...
Panel {
    ...
    function applyUpdateValue(value) {
        ...
        if (parsed !== updateMs)
            persistPluginSetting("updateMs", parsed);
        return true;
    }
    ...
    function finishUpdateEditing(apply, restoreFocus) {
        ...
        updateEditing = false;
        if (apply && parsed !== null)
            applyUpdateValue(parsed);
        else
            updateDraft = String(updateMs);
    ...
    function nextChoice(choices, current, direction) {
        ...
        if (index < 0)
            return direction > 0 ? choices[0] : choices[choices.length - 1];
        return choices[(index + direction + choices.length) % choices.length];
    }

    function cycleSetting(index, direction) {
        if (index === 0) {
            persistPluginSetting("iconStyle", nextChoice(["Meters", "CPU", "Pulse", "Custom"], iconStyle, direction));
            return;
        }
        ...
        PanelKeyCatcher {
            ...
            TapHandler {
                ...
                onTapped: function (eventPoint) {
                    if (!root.updateEditing)
                        return;
                    var point = updateRow.mapFromItem(keyCatcher, eventPoint.position.x, eventPoint.position.y);
            ...
            Column {
                ...
                Column {
                    ...
                    Column {
                        ...
                        RowLayout {
                            ...
                            TextField {
                                ...
                                foreground: root.foreground
                                font.family: root.fontFamily
                                hasCursor: !activeFocus && root.settingsIndex === root.customPathIndex
                        ...
                        Text {
                            visible: root.customIconError !== "" || root.customIconLoadFailed
                            width: parent.width
                            text: root.customIconError !== "" ? root.customIconError : "The custom icon could not be loaded."
                            color: root.foreground
                            font.family: root.fontFamily
                    ...
                    CursorSurface {
                        ...
                        RowLayout {
                            ...
                            TextField {
                                ...
                                enabled: root.updateAvailable
                                foreground: root.updateDraftInvalid ? root.urgent : root.foreground
                    ...
                    Text {
                        ...
                        width: parent.width
                        text: "Use a whole number from " + UpdateInterval.minimum + " to " + UpdateInterval.maximum + " ms."
                        color: root.urgent
                        font.family: root.fontFamily
                        font.pixelSize: Style.font.caption
                        wrapMode: Text.WordWrap
