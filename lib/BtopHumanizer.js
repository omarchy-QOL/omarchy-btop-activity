...
function vramText(used, total, label) {
  var suffix = " (" + (label || "vRAM") + ")"
  if (typeof used !== "number" || typeof total !== "number"
      || !(used >= 0 && total > 0)) return "--" + suffix
  var usedText = floatingHumanizer(used)
  var totalText = floatingHumanizer(total)
  if (usedText === "" || totalText === "") return "--" + suffix
  return usedText + "/" + totalText + suffix
}
