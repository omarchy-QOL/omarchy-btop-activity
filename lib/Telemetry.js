...
function emptyGpu(device, metadata) {
  ...
  return {
    ...
    defaultRenderer: info.defaultRenderer === true,
...
function fastfetch(data, gpus, metadata) {
  ...
  module.result.forEach(function(row) {
    ...
    metadata[gpu.id] = {
      name: row.name || "", memoryKind: kind, defaultRenderer: false
    }
    gpu.name = row.name || ""
    gpu.memoryKind = kind
    gpu.defaultRenderer = false
  ...
  var openGl = data.find(function(item) { return item.type === "OpenGL" })
  var result = openGl && openGl.result || {}
  var renderer = String(result.renderer || "").toLowerCase()
  var matches = renderer ? gpus.filter(function(gpu) {
    return gpu.name && renderer.indexOf(gpu.name.toLowerCase()) >= 0
  }) : []
  if (matches.length !== 1) {
    var vendor = String(result.vendor || "").toLowerCase()
    var names = { "0x10de": "nvidia", "0x1002": "amd", "0x8086": "intel" }
    matches = vendor ? gpus.filter(function(gpu) {
      return names[gpu.vendor] && vendor.indexOf(names[gpu.vendor]) >= 0
    }) : []
  }
  if (matches.length !== 1 || !metadata[matches[0].id]) return
  matches[0].defaultRenderer = true
  metadata[matches[0].id].defaultRenderer = true
}
